package de.tum.cit.aet.thesis.core.user.repository;
...
public interface UserRepository extends JpaRepository<User, UUID> {
	...
	/**
	 * Checks whether a user appears in any publicly visible context: as a supervisor or examiner
	 * on a non-anonymized thesis, as a student on a finished PUBLIC thesis, as a role holder on an
	 * open topic, or as a research group head.
	...
	@Query("""
			...
			WHERE u.id = :userId AND (
				EXISTS (
					...
					AND tr.id.role <> de.tum.cit.aet.thesis.thesis.constants.ThesisRoleName.STUDENT
				...
				OR EXISTS (
					...
					JOIN tr.thesis t
					WHERE tr.user.id = :userId
					AND t.visibility = de.tum.cit.aet.thesis.thesis.constants.ThesisVisibility.PUBLIC
					AND t.state = de.tum.cit.aet.thesis.thesis.constants.ThesisState.FINISHED
					AND t.anonymizedAt IS NULL
				)
				OR EXISTS (
