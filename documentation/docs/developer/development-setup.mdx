...
5. Findings are collapsed into a single feedback string (bold title, description, first location hint) with the AI category/severity preserved in dedicated columns. Every row is pinned to `document_version_id` — the id of the proposal/thesis file that was reviewed — so later uploads never silently re-target older AI comments.
