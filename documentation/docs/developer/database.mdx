...
sidebar_position: 2
description: Liquibase-managed schema changes, PostgreSQL image tag, and manual data-fixture SQL.
---
...
1. Create a new changeset by adding a SQL script in the [
   `/server/src/main/resources/db/changelog/changes`](https://github.com/ls1intum/thesis-management/blob/develop/server/src/main/resources/db/changelog/changes)
   directory.
2. Use descriptive and unique filenames (e.g. `09_new_feature_name.sql`).
3. Include the new file in the [
   `db.changelog-master.xml`](https://github.com/ls1intum/thesis-management/blob/develop/server/src/main/resources/db/changelog/db.changelog-master.xml) to
...
- [
  `insert_tum_aet_research_group.sql`](https://github.com/ls1intum/thesis-management/blob/develop/server/src/main/resources/db/changelog/manual/insert_tum_aet_research_group.sql)
...
The PostgreSQL image tag is **`18.4-alpine`**. The committed [`.env`](https://github.com/ls1intum/thesis-management/blob/develop/.env) (`POSTGRES_IMAGE_TAG`)
...
|----------|------------------------|------------------|
| `docker-compose.yml`, `docker-compose.prod.yml`, `docker-compose.showcase.yml` | committed [`.env`](https://github.com/ls1intum/thesis-management/blob/develop/.env) (auto-loaded); `.env.prod` for production | the `${POSTGRES_IMAGE_TAG:-18.4-alpine}` inline fallback in each compose file |
| Integration-test Testcontainers (`BaseIntegrationTest`, `BaseKeycloakIntegrationTest` via `TestContainerImages`) | committed [`.env`](https://github.com/ls1intum/thesis-management/blob/develop/.env), parsed in [`server/build.gradle`](https://github.com/ls1intum/thesis-management/blob/develop/server/build.gradle) (or a `POSTGRES_IMAGE_TAG` env var) | the `System.getProperty(…, "18.4-alpine")` fallback in `TestContainerImages` |
...
**To bump the version**, edit `POSTGRES_IMAGE_TAG` in the committed [`.env`](https://github.com/ls1intum/thesis-management/blob/develop/.env) — that updates
