...
| **Interviews** | Examiner overview with interview topics + upcoming interviews, process detail with interviewees + filter tabs (All/Uncontacted/Invited/Scheduled/Completed), supervisor access, examiner2 views, student denied, **scoring interviewees** (SegmentedControl + notes), **add slot modal** (length + date), interview slots section, **creating new interview process** (topic search, applicant selection), **student interview booking page** (booked slot with topic info + cancel, slot selection UI, completed process message) | Actual slot booking/cancellation, adding/inviting interviewees, accepting applicants from interviews |
...
1. **Actual file upload/download** — The file management UI (upload controls, file types table, upload history) is verified, but uploading and downloading actual files during the WRITING phase is not tested.
2. **Actual interview slot booking/cancellation** — The booking page UI is tested (booked state, slot selection, completed process), but the full book → cancel → rebook flow is not exercised.
3. **Editing group information** — Research group settings cards are verified as present, but changing values (auto-reject toggle, proposal phase toggle, presentation duration) and saving is not tested.
