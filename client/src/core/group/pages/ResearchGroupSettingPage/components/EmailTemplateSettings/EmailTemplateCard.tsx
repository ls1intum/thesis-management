import { Badge, Card, Text, Stack, Title, Group, Button, Flex, Tooltip } from '@mantine/core'
import type { IEmailTemplate } from '@/core/group/requests/responses/emailtemplate'
import { useState } from 'react'
import EmailTemplatePreviewModal from '@/core/group/pages/ResearchGroupSettingPage/components/EmailTemplateSettings/EmailTemplatePreviewModal'
          ...
          <Stack gap={'0.5rem'}>
            <Group align='center' wrap='nowrap' gap={'xs'}>
              ...
              >
                {disabled
                  ? 'Disabled'
                  : emailTemplate.researchGroupTemplate
                    ? 'Customized'
                    : 'Default'}
              </Badge>
            </Group>
  ...
  return (
    <>
      {disabled && disabledReason ? (
        <Tooltip label={disabledReason} multiline w={300}>
          <div>{cardContent}</div>
        </Tooltip>
      ) : (
        cardContent
